...
Battle.executeMultiple = function (characterA, characterB, battleCount, battleResults, config, cb, isInit) {
	...
	csv.stringify(battle.rows, function (err, output) {
		...
		if (battleCount > 0) {
			Battle.executeMultiple(characterA, characterB, battleCount - 1, battleResults, config, cb);
		} else {
			// done!
